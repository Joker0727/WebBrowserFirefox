...
namespace Gecko
{
    ...
    public static class XULRunnerLocator
    {
        ...
        private static string GetXULRunnerLocationWindows(string hintPath)
        {
            ...
            {
                string path = System.IO.Path.Combine(programFiles, folderSearch[i]);
        ...
        public static string GeckoHintPath(string version)
        {
            ...
            {
                dir = dir.Parent;
                var fullPath = Path.Combine(dir.FullName, Gecko.MyFun.Combine("packages", "Geckofx45." + extraInfo, "content", "Firefox"));
                if (Directory.Exists(fullPath))
                    return fullPath;
                fullPath = Path.Combine(dir.FullName, Gecko.MyFun.Combine("packages", "Geckofx45." + extraInfo.TrimEnd('0').TrimEnd('.'), "content", "Firefox"));
                if (Directory.Exists(fullPath))
                    return fullPath;
