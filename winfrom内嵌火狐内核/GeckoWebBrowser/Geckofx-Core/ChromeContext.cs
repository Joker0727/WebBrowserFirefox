...
namespace Gecko
{
    internal sealed class ChromeContext : IDisposable
    {
        ...
        public ChromeContext()
        {
            ...
            {
                webNav = Gecko.Interop.ExtensionMethods.AsComPtr(appShallSvc.Instance.CreateWindowlessBrowser(true));
        ...
        private void Init()
        {
            ...
            {
                _isInitialized = true;
                GeckoDomDocument doc = Gecko.Interop.ExtensionMethods.Wrap(webNav.Instance.GetDocumentAttribute(), 
                    GeckoDocument.CreateDomDocumentWraper);
...
";
                ...
                command = Gecko.Interop.ExtensionMethods.AsComPtr(Xpcom.QueryInterface<nsIDOMXULElement>(button.DOMElement));
            }
        }
        ...
        internal byte[] DrawWindow(nsIDOMWindow window, uint x, uint y, uint w, uint h)
        {
            ...
            {
                using (var drawResult = Gecko.Interop.ExtensionMethods.AsComPtr(command.Instance.SetUserData(key, null)))
                {
                    if (drawResult != null)
